...
export default class TransactionCoordinator  {
  ...
  constructor(caver, poolConfig,   mongoInterface )
  {
    ...
    //web3 might be for mainnet 

    this.paymentContract = ContractHelper.getBatchedPaymentsContract(this.poolConfig )
  ...
  */
    ...
    try{ 
      ...
      let payment_txes = await TransactionHelper.findTransactionsWithQuery({ txType:'batched_payment', status:'pending' } ,mongoInterface)
       ...
       if( payment_txes != null && payment_txes.length > 0) {
        ...
        TransactionHelper.checkTransactionReceipts( payment_txes, mongoInterface)
      }
  ...
  static async monitorMinedSolutions( mongoInterface, poolConfig )
  {
   ...
   if( solution_txes != null && solution_txes.length > 0)
    {
      //look for receipt 
       var response = await TransactionHelper.checkTransactionReceipts( solution_txes , mongoInterface)
    }
   }catch(e)
   {
   console.log('error',e)
...
/*
   ...
   async requeueSkippedBatchedPaymentTransactions(mongoInterface, poolConfig){
    ...
    for(let pendingTransaction of pendingTransactions){
      ...
      if(!pendingTransaction.block || pendingTransaction.block < currentEthBlock-500){
        ...
        let paymentWithUUIDWasSuccessful = await this.checkPaymentWithUUIDWasSuccessful(  pendingTransaction.txData.uuid  ) 

        if(paymentWithUUIDWasSuccessful){

          await mongoInterface.updateOne('transactions',{_id:pendingTransaction._id}, 
            {block: currentEthBlock, status:'completed'} )
   ...
   */
   ...
   async checkPaymentWithUUIDWasSuccessful( uuid ){

    let from = this.poolConfig.paymentsConfig.publicAddress

   // console.log('payments contract', this.paymentContract)

    let result = await this.paymentContract.methods.paymentSuccessful( from,  uuid ).call()
...
*/
   ...
   //fix me up ! 
      ...
      //this could be messed up since some tx go to polygon , not mainnet ! might not ever see them 


     async requestTransactionData(tx_hash)
     {
       try{
          var data = await this.caver.transaction.getTransactionByHash(tx_hash);
        }catch(err)
        {
   ...
   async requestTransactionReceipt(tx_hash)
   {
      ...
      try{
      var receipt = await this.caver.rpc.klay.getTransactionReceipt(tx_hash);
      }catch(err)
      {
