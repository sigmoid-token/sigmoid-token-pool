...
export default class PeerInterface  {
  ...
  constructor(mongoInterface, poolConfig){
    ...
    this.tokenContract = ContractHelper.getTokenContract( this.poolConfig  )  
...
/* 
   ...
   */
   ...
   static async handlePeerShareSubmit(shareData ,mongoInterface, poolConfig)
   {
     ...
     */
     ...
     var digest =  cav.utils.soliditySha3( poolChallengeNumber , poolEthAddress, nonce )
     

     LoggingHelper.appendLog( ['got new share from miner ', minerEthAddress, ' ', poolChallengeNumber , poolEthAddress, nonce, digest], LoggingHelper.TYPECODES.SHARES, mongoInterface)
     ...
     var minShareTarget = cav.utils.toBN(PeerHelper.getPoolMinimumShareTarget(poolConfig) ) ;
     var miningTarget = cav.utils.toBN(await TokenDataHelper.getPoolDifficultyTarget(mongoInterface) ) ;
    ...
    if(difficulty >= minShareDifficulty){
        ...
        //quick fix to prevent miner lying about difficulty, thanks crnxhh
        if(difficulty > minShareDifficulty){
  ...
  static async  handleValidShare( nonce,minerEthAddress,digest,difficulty, mongoInterface, poolConfig )
   {
      ...
      { 
       ...
       //  await PeerHelper.saveMinerDataToRedisMongo(minerEthAddress,minerData, mongoInterface)
        ...
        var difficultyBN = cav.utils.toBN(difficulty);
        ...
        await mongoInterface.updateOne('minerData', {_id: minerData._id},  {lastSubmittedSolutionTime: PeerHelper.getTimeNowSeconds()}    )
          ...
          //--optional 
        var shareCredits =  await PeerHelper.getShareCreditsFromDifficulty( difficulty,poolConfig )
  ...
  async initJSONRPCServer()
     {
       ...
       console.log('listening on JSONRPC server localhost:8080')
        ...
        submitShare: async function(args, callback) {
          ...
          var challenge_number = args[2];
          var custom_vardiff_used = args[3];
          ...
          var difficulty = PeerHelper.getPoolMinimumShareDifficulty(poolConfig)  ;
         ...
         /* var maxShareDifficulty = PeerHelper.getPoolMaximumShareDifficulty(poolConfig)  ;
          ...
          LoggingHelper.appendLog( [ 'adding share to queued shares list', digest], LoggingHelper.TYPECODES.SHARES, mongoInterface)
          ...
          callback(null,  validJSONSubmit );
           ...
           getMinerData: async function(args, callback) {
             ...
             if(cav.utils.isAddress(minerEthAddress.toString()) ){
