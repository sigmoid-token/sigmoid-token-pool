...
/*
   ...
   */
   ...
   var tokenContract =  ContractHelper.getTokenContract(poolConfig)
   var mintMethod = tokenContract.methods.mint(solution_number)
   
   //var mintMethod = this.mintHelperContract.methods.proxyMint(solution_number,challenge_digest);

  try{
    var txCount = await cav.rpc.klay.getTransactionCount(addressFrom);
   ...
   var addressTo = tokenContract.options.address;
    ...
    var txData = cav.abi.encodeFunctionCall({
            name: 'mint',
            type: 'function',
            inputs: [{
                type: 'uint256',
   ...
   /* let mintingGasPriceBoost = 0
    ...
    if(transactionType == "0x02"){
      ...
      txOptions.gasLimit= cav.utils.toHex(estimatedGasCost)
      txOptions.maxFeePerGas= cav.utils.toHex( cav.utils.convertToPeb( solutionGasPriceWei.toString() , 'Gpeb' ) )
      txOptions.maxPriorityFeePerGas= cav.utils.toHex( cav.utils.convertToPeb( priorityGasPriceWei.toString() , 'Gpeb' ) )

    }else{

      txOptions.gas = cav.utils.toHex(estimatedGasCost)
      txOptions.gasPrice = cav.utils.toHex(cav.utils.convertToPeb(solutionGasPriceWei.toString(), 'Gpeb') )
    ...
    LoggingHelper.appendLog(['sendSignedRawTransactionSimple',txOptions], LoggingHelper.TYPECODES.TRANSACTIONS, mongoInterface)
      ...
      let txResult = await TransactionHelper.sendSignedRawTransactionSimple(txOptions,addressFrom,privateKey);
  ...
  static async submitBatchedPayment(txData, mongoInterface, poolConfig)
  {
    ...
    var tokenContract =  ContractHelper.getPaymentsTokenContract(poolConfig)
    ...
    var batchedPaymentsContract =  ContractHelper.getBatchedPaymentsContract(poolConfig)
   ...
   /* const txOptions = {
    ...
    if(transactionType == "0x02"){
      ...
      txOptions.type = transactionType
      txOptions.gasLimit= cav.utils.toHex(estimatedGasCost)
      txOptions.maxFeePerGas= cav.utils.toHex( cav.utils.convertToPeb( paymentsGasPriceWei.toString() , 'Gpeb' ) )
      txOptions.maxPriorityFeePerGas= cav.utils.toHex( cav.utils.convertToPeb( priorityGasPriceWei.toString() , 'Gpeb' ) )

    }else{

      txOptions.gas = cav.utils.toHex(estimatedGasCost)
      txOptions.gasPrice = cav.utils.toHex(cav.utils.convertToPeb(paymentsGasPriceWei.toString(), 'Gpeb') )
  ...
  static  async requestTransactionReceipt(tx_hash)  //not working
  {
      try{
       var receipt = await  cav.rpc.klay.getTransactionReceipt(tx_hash);
     }catch(err)
     {
    ...
    static async checkTransactionReceipts(txes, mongoInterface)
    {
      ...
      for(let transaction of txes)
      {
        ...
        {
          var liveTransactionReceipt = await TransactionHelper.requestTransactionReceipt(tx_hash)
          ...
          if(liveTransactionReceipt != null )
          {
            ...
            LoggingHelper.appendLog(['got receipt',liveTransactionReceipt], LoggingHelper.TYPECODES.TRANSACTION, mongoInterface)
                ...
                var transaction_succeeded = ((liveTransactionReceipt.status == true)
                                               || (cav.utils.hexToNumber( liveTransactionReceipt.status) == 1 ))
    ...
    static transactionWasMined(transaction){
     return (transaction.status == 'success' || transaction.status == 'reverted')
      ...
      static  async requestCurrentChallengeNumber()
       ...
       static async sendSignedRawTransactionSimple(txOptions,addressFrom,pKey){
          
         
        
          let signedTx = await new Promise((resolve, reject) => {
          cav.klay.signTransaction(txOptions);
        });
    
    
    
          let submittedTx = await new Promise((resolve, reject) => {
            cav.transaction.sendSignedTransaction(signedTx.rawTransaction)
