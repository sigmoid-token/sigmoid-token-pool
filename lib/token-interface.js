...
//    this.web3 = new Web3(poolConfig.mintingConfig.web3Provider);
    ...
    this.mintingWeb3 = new Web3(poolConfig.mintingConfig.web3Provider);
    this.paymentsWeb3 = new Web3(poolConfig.paymentsConfig.web3Provider);
    this.caver = cav;
 ...
 async  update()
  {
   ...
   let transactionCoordinator = new TransactionCoordinator(this.caver,this.poolConfig,  this.mongoInterface  )

   transactionCoordinator.update();

    
   
    const tokenContract = await ContractHelper.getTokenContract( this.poolConfig  )  
    ...
    cron.schedule('*/20 * * * * *', async function() {
      ...
      await TokenDataHelper.collectTokenParameters( tokenContract, this.mongoInterface) 
...
//converted all time miner balance into tokens awarded and creates 'balance payment' records which will get batch-paid 
 static async buildBalancePayments(mongoInterface, poolConfig )
  { 
    ...
    for(let minerData of minerList) //reward each miner
    {
      ...
      if(typeof minerData.tokensAwarded == 'undefined') minerData.tokensAwarded = 0;
       ...
       && cav.utils.isAddress( minerAddress )  
       )
       {
  ...
  static async buildBatchedPaymentTransactions(mongoInterface, poolConfig )
  {
    ...
    let newBatchedPaymentTxData = { uuid: cav.utils.randomHex(32) , payments:[]}  
    ...
    balancePaymentsArray.filter( bp => TokenInterface.balancePaymentIsValid( bp )  );
  ...
  static balancePaymentIsValid( balancePayment ){
    ...
    return cav.utils.isAddress( balancePayment.minerEthAddress )  
  ...
  { 
    ...
    for(let pendingBalancePayment of pendingBalancePayments){
      ...
      if(matchingTransaction && matchingTransaction.txHash){
        ...
        await mongoInterface.updateOne('balance_payments',{_id: pendingBalancePayment._id}, {txHash: matchingTransaction.txHash, status: 'success'} )
